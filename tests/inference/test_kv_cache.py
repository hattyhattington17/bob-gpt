...
def test_update_writes_at_cache_position() -> None:
    ...
    k_all, v_all = cache.update(0, k, v, cache_position=0)
...
def test_second_update_appends_at_next_position() -> None:
    ...
    cache.update(0, k1, v1, cache_position=0)
    k2 = torch.randn(1, 2, 1, 8)
    v2 = torch.randn(1, 2, 1, 8)
    k_all, _ = cache.update(0, k2, v2, cache_position=4)
...
def test_layers_share_position_not_data() -> None:
    ...
    cache.update(0, k0, k0, cache_position=0)
    k1 = torch.randn(1, 2, 3, 8)
    a, _ = cache.update(1, k1, k1, cache_position=0)  # same window, layer 1's own data
...
def test_overflow_raises() -> None:
    ...
    cache.update(0, k, k, cache_position=0)
    with pytest.raises(ValueError):
        cache.update(0, k, k, cache_position=3)  # 3 + 3 > 4
